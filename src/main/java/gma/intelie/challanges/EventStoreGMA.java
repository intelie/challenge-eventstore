...
public class EventStoreGMA implements EventStore {
	...
	public EventIterator query(String type, long startTime, long endTime) {
		...
		Iterator<Event> itEvento;
		List<Event> lstEvent = htEvento.get(type);
		
		synchronized (lock) {
			if (!htEvento.containsKey(type)) {
				return EventIteratorGMA.create(Collections.emptyIterator(), lstEvent,lock);
			}

			 itEvento = lstEvent.stream().filter(e -> {
				return (e.timestamp() >= startTime && (e.timestamp() < endTime));
			}) // .distinct()
					.sorted((p1, p2) -> (p1.timestamp() <= p2.timestamp()) ? -1 : 1).collect(Collectors.toList()) // snapshort
																													// -
																													// cursor
																													// imutavel
					.iterator();
		}
		return EventIteratorGMA.create(itEvento, lstEvent, lock);
	}
